...
from vitaminc.modeling.custom_trainer import NewTrainer
...
class VitCTrainingArgs(TrainingArguments):
    ...
    loss_fn: Optional[str] = field(
        default=None, metadata={"help": "Which of the Utama loss functions to use."}
    )

    bias_name: Optional[str] = field(
...
def main():
    ...
    trainer = NewTrainer(
        ...
        loss_fn_name = training_args.loss_fn
    )
    ...
    if training_args.do_test or training_args.do_predict:
        if training_args.test_on_best_ckpt:
            with open(output_best_ckpt_file, "r") as reader:
                ...
                model = AutoModelForSequenceClassification.from_pretrained(best_ckpt, output_hidden_state=False)
